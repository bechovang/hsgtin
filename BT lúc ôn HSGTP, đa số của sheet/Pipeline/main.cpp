...
ll sum(ll n){
    return (n * (n+1) ) /2; // ct cấp số cộng
}
ll sum(ll l, ll r){
    if (l<=1)
        return sum(r);
    else
        return sum(r) - sum(l-1) ; // ct prefix sum
}

int tknp_minSplitters(ll n, ll k){
    ll l=1; ll r=k;
    while (l<=r){ // l mà trùng r là chưa vòng lặp. do r=mid-1, nếu r=mid thì while (l<r)
        ll mid = l + (r-l)/2;
        if (sum(mid,k)==n){
            return k-mid+1; // trả về số phần tử từ k đến mid
        }
        // do đang tìm sum(mid,k) lớn nhất nên
        // nếu sum(mid,k)>x thì cần sum nhỏ lại
        //  => cần thu hẹp mid đến k
        //  => check đoạn bên phải <=> mid mới nằm trong khoảng [mid+1,r]
        // nếu sum(mid,k)<x thì cần sum lớn hơn
        //  => cần mở rộng mid đến k
        //  => check đoạn bên trái <=> mid mới nằm trong khoảng [l,mid-1]

        //=> nếu ko tìm được sum(mid,k)==n
        //   thì cũng tìm được sum(mid,k) lớn nhất
        //   vì nếu sum lớn thì thu nhỏ nó, sum nhỏ thì mở rộng nó
        if (sum(mid,k)>n){
            l=mid+1;
        }
        if (sum(mid,k)<n){
    ...
    // ko tìm thấy sum(mid,k)==n
    // lấy l-k+1 là x phần tử cuối vì trong tknp
    //      nếu sum thỏa yêu cầu l sẽ tăng lên
    //      r chỉ đóng vai trò thu hẹp sum lại để tìm kiếm nhanh hơn
    //      => l sẽ tăng đến khi nào tg là lớn nhất
    //      => chọn l-k+1 là x phần tử cuối
    // => n = x phần tử cuối + 1 phần tử nữa
    // => trả về (số phần tử từ k đến mid) + 1
    return k-l+2;
}
